function [ Score,Cell_Score_Array,Time_Locations,P_val,Deviation_Score] = Distance_Change_Score_From_Perm_After_event(perm, events,downsample,bin,str)
%For a specific cell, this will give back the DISTANCE score based on the
%Permutation
%****If the score is negative, that implies the animals got closer
%If the score is postive, that implies the animals got further away
...
%Inputs:
    ...
    %str = either 'Novel', 'Partner', 'All', or 'Non'  Each refers to a
        ...
        %or Partner.  The sting allows us to pull out the data that is only in
        %the correct chamber
...
%downsample
    ...
    P_val = ((length(find(d < Score)))/perm)*100;
else
    Deviation_Score = NaN;
    P_val = NaN;
